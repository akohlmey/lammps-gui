...
crashes). Mitigation (DONE): imported only a **lightweight subset** of Lepton
(dropped the asmjit JIT and the `CompiledExpression`/`CompiledVectorExpression`
path -- we only need parse + evaluate + differentiate, which the interpreted
`ExpressionProgram` still provides) and placed it in a **dedicated namespace**,
**`LeptonMini`**, so its symbols cannot collide with the LAMMPS-provided ones.
It lives under `thirdparty/lepton_mini/` (umbrella header `lepton_mini.h`,
