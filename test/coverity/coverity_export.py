...
README.md (cookie capture, troubleshooting).
...
Customized for LAMMPS-GUI: Coverity Scan project akohlmey/lammps-gui (projectId
16333, all-defects viewId 67542, served on the scan8 shard).  Those are the
built-in defaults below, so the -p / -v / --*-url flags are only needed if the
project id, view id, or server shard change.  Adapted from the LAMMPS
distribution (tools/coverity, PR lammps/lammps#5057).
...
DEFAULT_VIEWS_URL = "https://scan8.scan.coverity.com/views/table.json"
DEFAULT_REPORTS_URL = "https://scan8.scan.coverity.com/reports/table.json"
...
DEFAULT_TRACE_URL = (
    "https://scan8.scan.coverity.com/sourcebrowser/source.json"
    ...
    while True:
        ...
        if new == 0:
            sys.exit("\nPAGINATION PROBLEM: a full page added 0 new rows -- the "
                     ...
                     "(same session?). Re-capture the requests (README.md).")
        k += 1
        if args.delay:
...
def cmd_traces(args):
    ...
    for n, cid in enumerate(cids, 1):
        ...
        combined.append({"cid": cid, "file": payload.get("filePath"),
                         ...
                         "events": res["events"]})
        short = (payload.get("filePath") or "").lstrip("/")
...
def main(argv=None):
    ...
    d.add_argument("-p", "--project-id", default="16333",
                   help="projectId from the request body (default: 16333, the "
                        "LAMMPS-GUI project).")
    d.add_argument("-v", "--view-id", default="67542",
                   help="viewId from the request body (default: 67542, the "
                        "LAMMPS-GUI view that lists all defects).")
    ...
    t.add_argument("--project-id", default="16333")
